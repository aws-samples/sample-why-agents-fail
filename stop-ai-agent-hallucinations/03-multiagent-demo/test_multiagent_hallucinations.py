...
def print_tokens(result, label):
    """Print Strands-native token usage for a single agent result."""
    ...
    if result.metrics:
        ...
        print(f"💰 {label} tokens: {u['inputTokens']} in, {u['outputTokens']} out, {u['totalTokens']} total")


def swarm_tokens(result, label):
    """Sum and print token usage across all agents in a swarm result."""
    # 📊 In a Swarm each agent has its OWN metrics. We iterate over result.results
    # (executor → validator → critic) and sum each one's accumulated_usage to
    # see the TOTAL cost of the multi-agent system (usually several times a single agent's).
    total = {"input": 0, "output": 0, "total": 0}
    for _node_id, node_result in result.results.items():
        if node_result.result.metrics:
            u = node_result.result.metrics.accumulated_usage
            total["input"] += u["inputTokens"]
            total["output"] += u["outputTokens"]
            total["total"] += u["totalTokens"]
    print(f"💰 {label} tokens (all agents): {total['input']} in, {total['output']} out, {total['total']} total")
...
print_tokens(result, "Single agent")
...
swarm_tokens(result, "Multi-agent")
